...
- локальный `origin` пока не подключен
...
- подключение `origin`
- настройка способа аутентификации для `push` в GitHub
...
Следующая реальная инженерная задача - привязать локальный репозиторий к GitHub и создать Laravel-каркас, чтобы уже работать от документации к коду.
