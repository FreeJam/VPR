...
- teacher pages для учеников и групп
- назначения учитель -> ученик
- назначения учитель -> группа
...
4. При необходимости открыть `Ученики` и `Группы` и собрать группу.
5. Нажать `Назначить` и выдать работу ученику или группе.
6. Войти как `student@vpr.local`, открыть `Назначения`, пройти попытку и отправить работу.
7. Вернуться под учителем в `Проверка`, выставить баллы и сохранить review.
